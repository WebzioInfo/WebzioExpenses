...
function EntriesContent() {
  ...
  const [founderView, founderViewMode] = useState('Company'); // 'Company' or 'Personal'

  const filtered = useMemo(() => {
    if (!entries) return [];
    let list = [...entries];

    // RBAC: Staff only see their own records
    if (!isManagement) {
      list = list.filter(t => String(t.personId) === String(user?.staffId));
    }

    // Founder Portal: Toggle between Company and Personal
    ...
    <div className="space-y-8 py-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-1">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={() => exportCSV(filtered)} icon={Download}>Export Ledger</Button>
          {isManagement && <Button onClick={() => router.push('/add-transaction')} icon={Plus}>Record Entry</Button>}
        </div>
      </div>
      ...
      <Card className="p-8 border border-accounting-text/5 shadow-2xl space-y-8">
        <div className="flex flex-col xl:flex-row gap-6">
          ...
          <div className="space-y-4 lg:min-w-[400px]">
            <label className="text-[10px] font-black text-secondary-text uppercase tracking-widest px-1">
               {isFounder ? 'Context Matrix' : 'Temporal Scope'}
            </label>
            <div className="flex p-1.5 bg-accounting-bg/40 rounded-2xl border border-white -inner">
              {isFounder ? (
                ['Company', 'Personal'].map(m => (
                  <button
                    key={m}
                    onClick={() => founderViewMode(m)}
                    className={cn(
                      "flex-1 h-11 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all",
                      founderView === m ? "bg-accounting-text text-white shadow-lg" : "text-secondary-text/30 hover:text-accounting-text"
                    )}
                  >
                    {m} Ledger
                  </button>
                ...
                <td className="w-24">
                  {isManagement && (
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all">
                      ...
                      {isFounder && (
                        <Button variant="ghost" size="sm" icon={Trash2} onClick={() => { if (confirm('Permanent System Purge?')) deleteEntry(entry.id); }} className="w-10 h-10 p-0 text-rose-400 hover:text-rose-600 bg-white border border-accounting-text/5 shadow-sm" />
                      )}
                    </div>
                  )}
                </td>
